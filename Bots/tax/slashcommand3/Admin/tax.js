...
.setFooter({text:interaction.user.username , iconURL:interaction.user.displayAvatarURL({dynamic:true})})
    ...
    .setColor('Random')
    let btn1 = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dis').setLabel(`${interaction.guild.name}`).setEmoji('✨').setStyle(ButtonStyle.Secondary).setDisabled(true))
    return interaction.reply({embeds:[embed1] , components : [btn1]})
}
}